...
function modelTierLookup(raw: string): number {
  let id = raw.replace(/^(openai\/|anthropic\/|google\/)/, '').replace(/-\d{4}-\d{2}-\d{2}$/, '').toLowerCase().trim();
  // Claude's API returns hyphenated versions (claude-opus-4-7); our table uses
  // dots (claude-opus-4.7). Without this, claude-opus-4-7 (7.5×) falls through
  // to the substring claude-opus-4 (1×) and the flagship reads as a base model.
  if (id.startsWith('claude-')) {
    id = id.replace(/-\d{8}$/, '').replace(/(\d)-(\d)/, '$1.$2');
  }
  // Drop reasoning-effort suffixes (claude-opus-4.7-high → claude-opus-4.7).
  id = id.replace(/-(xhigh|extra-high|max|high|medium|med|low|minimal)$/, '');
  // Exact match wins; otherwise the LONGEST matching key (so claude-opus-4.7
  // beats the shorter claude-opus-4 substring regardless of iteration order).
  if (MODEL_TIERS[id] !== undefined) return MODEL_TIERS[id];
  let best = 0, bestLen = 0;
  for (const [k, v] of Object.entries(MODEL_TIERS)) {
    if (k.length > bestLen && id.includes(k)) { best = v; bestLen = k.length; }
  }
  return best;
}
  ...
  'capsLetterRatio': (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      for (let i = 0, len = Math.min(text.length, 2000); i < len; i++) {
        ...
        // кириллица: заглавные А-Я + Ё / строчные а-я + ё
        else if ((c >= 0x0410 && c <= 0x042F) || c === 0x0401) { letters++; upper++; }
        else if ((c >= 0x0430 && c <= 0x044F) || c === 0x0451) { letters++; }
      }
  ...
  'capsWordRatio': (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      ...
      for (let i = 0; i <= scanLen; i++) {
        ...
        if (c === 32 || c === 9 || c === 10 || c === 13) {
          ...
          wordLen++;
          // заглавные: латиница A-Z + кириллица А-Я/Ё
          if ((c >= 65 && c <= 90) || (c >= 0x0410 && c <= 0x042F) || c === 0x0401) hasLetter = true;
          // строчные: латиница a-z + кириллица а-я/ё
          else if ((c >= 97 && c <= 122) || (c >= 0x0430 && c <= 0x044F) || c === 0x0451) allUpper = false;
        }
      }
      // Один CAPS-токен (аббревиатура: «LLM», «API», «SQL») — не крик. Требуем
      // минимум два заглавных слова, иначе ratio в коротких фразах даёт ложняки
      // («А сохраняет LLM?» → 1 из 2 слов = 0.5).
      return wordCount > 0 && capsWordCount >= 2 ? capsWordCount / wordCount : 0;
    }
  ...
  'workTypeCount': (argNodes: ASTNode[], ctx: Record<string, unknown>) => {
    {
      const reqs = evaluate(argNodes[0], ctx);
      if (!Array.isArray(reqs)) return 0;
      // Catch-all buckets ('feature' = classifier default, 'other' = unclassified)
      // are present in almost every long session, so counting them inflates the
      // distinct-type total. Only genuine, named task types count as drift.
      const CATCH_ALL = new Set(['feature', 'other', '']);
      const types = new Set<string>();
      for (const r of reqs) {
        const msg = asStr((r as Record<string, unknown>)?.messageText);
        const wt = (r as Record<string, unknown>)?.workType;
        const t = typeof wt === 'string' && wt ? wt : classifyWorkText(msg);
        if (!CATCH_ALL.has(t)) types.add(t);
      }
      return types.size;
    }
  },

...
thresholds:
  ...
  minReqs: 2
patterns:
  frustration: ["!{3}", "\?{3}", "wtf|come on|why won't", "this is broken|doesn't work|still (not|doesn't)", "(?iu)(не работает|не получается|не получилось|не пашет|опять не|снова не|сломал|развалил)", "(?iu)(бесит|задолбал|задрал|достал[ои]?|надоел|издевае|прекрати|сколько можно|сколько раз|да сколько)", "(?iu)(что за (хрень|бред|фигн|дич|херн)|это бред|я же (сказал|просил|говорил|писал)|ты опять|опять ты)"]
---

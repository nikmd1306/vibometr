...
match: flatSumField(requests, "aiCode", "loc") >= thresholds.minAiLoc AND \
  ...
  matches(first(requests).messageText, "(?iu)(@[\\w./-]+|[\\w./-]+\\.md\\b|выполни (этот )?план|по плану|следуй плану|по нашему плану|implement (the|this) plan|follow (the|this) plan)") OR \
